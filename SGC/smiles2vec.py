...
def mol2graph(mols: Union[List[str], List[Chem.Mol]], atom_descriptors_batch: List[np.array] = None) -> BatchMolGraph:
    ...
    if atom_descriptors_batch is not None:
        return BatchMolGraph(
            [MolGraph(mol, atom_descriptors) for mol, atom_descriptors in zip(mols, atom_descriptors_batch)])
    else:
        return BatchMolGraph([MolGraph(mol) for mol in mols])


def rdkit_2d_normalized_features_generator(smiles) -> np.ndarray:
    """
...
# feats = mol2graph([mol1])
# print(type(Chem.GetAdjacencyMatrix(mol1)))
# print((feats.get_components()[0].shape))
# print(type((feats.get_components()[0]).numpy()))

train_smiles = []
labels = []
with open(dir + "test_set_filtered.csv") as f:
    f.readline()
    for line in f:
        smile = line.strip().split(",")[2]
...
for smile in train_smiles:
    ...
    atom_feat = mol2graph([mol]).get_components()[0].numpy()  # bond features 1
    atom_feat_list.append(atom_feat)
    aj_matrix = Chem.GetAdjacencyMatrix(mol)
    aj_matrix_list.append(aj_matrix)

# print(len(np.asarray(atom_feat_list)))
# print(len(np.asarray(aj_matrix_list)))

name = "test"
feat_file = "/Users/E/Desktop/STUDY/CMU/Second_Year/Fall_2020/11685_Deep_Learning/Project/data/" + name + "_feature.npy"
adj_file = "/Users/E/Desktop/STUDY/CMU/Second_Year/Fall_2020/11685_Deep_Learning/Project/data/" + name + "_adjacent_matrix.npy"

np.save(feat_file, atom_feat_list)
np.save(adj_file, aj_matrix_list)

train_label_file = "/Users/E/Desktop/STUDY/CMU/Second_Year/Fall_2020/11685_Deep_Learning/Project/data/" + name + "_label.npy"
